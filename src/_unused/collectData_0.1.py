...
bus = SMBus(2)
data1 = [0x85, # Precision selection
         0x83] # 
raw = 0
out = 0
...
while():#ADC.read(PWMReadPin) < 0.01):
    ...
    for x in range(sampleRate * wavFileDuration):
        a = bus.write_i2c_block_data(0x48, 0x01, data1) # start sample for I2C
        time.sleep((sampleRate ** -1))
        b = bus.read_i2c_block_data(0x48, 0x00, 2)  # read conversion register on ads1115
        raw = b[0]*256 + b[1]
        out = raw  * .0000625
        #print(out)
        adcData[x] = out;
